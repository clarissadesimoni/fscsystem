...
class MyTask:
    def __init__(self, id: Union[str, None] = None, api_obj: Union[Task, None] = None):
            ...
            self.subtasks: List[MyTask] = [MyTask(api_obj=st) for st in all_tasks_dict.get(self.id, [])]
...
class Section:
    ...
    def completion_count(self, count_mig=False, count_habits=True):
        ...
        if not count_mig and not count_habits:
            ...
            tmp = self.tasks
        tmp = [t.is_completed() for t in tmp]
...
class Project:
    ...
    def to_string(self, completed: bool = False, vc: bool = False):
        ...
        if not completed:
            comp_count = self.completion_count()
            comp = self.completion()
            res = f'**PROJECT: {self.name}** (Done: {comp_count[0]}/{comp_count[1]}: {comp:.2%}){(" " + generate_progress_bar(comp)) if comp_count[1] > 10 else (" " + generate_shorter_progress_bar(*comp_count)) if comp_count[1] > 1 else ""}'
        else:
...
def get_todoist(to_check: List[str] = [], to_filter: Union[List[str], str, None] = None) -> TaskList:
    ...
    missing: Set[str] = set(to_check) - {t.id for t in all_tasks}
    ...
    while i < len(all_tasks) - 1:
        if all_tasks[i].id == all_tasks[i + 1].id:
            ...
            i += 1
    all_tasks_dict = simplify_tasks(all_tasks)
    for task in all_tasks_dict[None]:
...
def update_data(tasks_to_insert, tasks_to_delete):
    global is_connected
    if is_connected:
        try:
